...
def load_activities_records():
    print("Started loading from activities files")
                ...
                if frame.frame_type == fitdecode.FIT_FRAME_DATA:
                    ...
                    if frame.name == "session":
                        ...
                        if frame.has_field("start_position_long"):
                            activity["longitude"] = frame.get_value(
                                "start_position_long"
                            )
                        ...
                        if frame.has_field("max_heart_rate"):
                            activity["max_heart_rate"] = frame.get_value(
                                "max_heart_rate"
                            )
    ...
    with open(get_path("activities_records.pkl"), "wb+") as f:
        pickle.dump(activities, f)

    print("Finished loading from activities files")


if not path.isfile(get_path("activities_records.pkl")):
    load_activities_records()


with open(get_path("activities_records.pkl"), "rb") as f:
...
df.to_csv(get_path("activities.csv"), index=False)
